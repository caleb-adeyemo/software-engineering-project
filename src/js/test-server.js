...
function test(){
   ...
   let match = SERVER.get_reservations(
      lot_0
      ,resrv => resrv.time.start.getTime() === t0.start.getTime()
   );


   assert.deepEqual(match.length,2,"get query should find the two reservations with identical time");

   match = SERVER.get_reservations(
      lot_0
      ,resrv => resrv.time.start.getTime() === t1.start.getTime()
   );
   ...
   match = SERVER.get_reservations(
      lot_0
      ,resrv => resrv.time.start.getTime() === t_fail.start.getTime()
   );
