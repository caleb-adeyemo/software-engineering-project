...
function test_reservations(){
   ...
   let resrv_arr = [];
   let test_resrv = reservations.reservation(test_time,test_space,test_user,test_vID);
   resrv_arr.push(test_resrv);
   console.log("====BASIC RESRV TEST====");
   let test_time1 = reservations.time(d1,one_hr);
   let test_resrv1 = reservations.reservation(test_time1,test_space,test_user,test_vID);
   let res = reservations.is_valid(resrv_arr,test_resrv1); 
   assert.deepEqual(res.code,reservations.RES_OK,'vaid reservations are accepted');
   resrv_arr.push(test_resrv1);
   console.log("====TEST OVERLAP====");
   let res1 = reservations.is_valid(resrv_arr,test_resrv); 
   console.log(res1); 
   assert.deepEqual(res1.code,reservations.TIME_COLL,'duplicate reservations are not accepted');
...
function test_spaces(){
   ...
   assert.deepEqual(spaces[5].level,"B","level is assigned in alphabetical order");
   assert.deepEqual(spaces[5].bay,"2","lot is indexed from 0 and reset by each level"); 
...
function test_times(){
   ...
   assert.deepEqual(t0.overlap(t1),false,"march 2nd 10:20 cannot overlap with march 3rd");
   assert.deepEqual(t0.overlap(t2),true,"time 10:40 is within the range 10:20-11:20");
}
